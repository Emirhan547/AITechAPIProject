...
namespace AITech.Business.Services.ProjectServices
{
    public class ProjectService(IProjectRepository _projectRepository, IUnitOfWork _unitOfWork) : IProjectService
    {
        ...
        {
            var project = await _projectRepository.GetByIdAsync(id);  // ✔ Task<Project> → Project
        ...
        {
           var projectsWithCategories= await _projectRepository.GetProjectsWithCategoriesAsync();
            return projectsWithCategories.Adapt<List<ResultProjectDto>>();
        }
